...
class SafelinkController extends Controller
{
    ...
    public function getPayment(Request $request, Ajax $ajax)
    {
        ...
        $body['referenceId'] = 'rawinfo_' . $rawInfo->id; //your reference id
    ...
    public function orderPlaced(Request $request, Ajax $ajax)
    {
        ...
        foreach ($products as $product) {
            ...
            $safelink->product_id = $product['prod_id'];
    ...
    public static function saveOrder($rid, $request)
    {
        ...
        foreach ($products as $product) {
            ...
            $safelink->status = 0;
            $safelink->save();
            array_push($new_safelinks, $safelink->id);
        }

        $billing_profile = BillingProfile::with(['newsafelinks', 'newsafelinks.master_product'])->find($billing_profile_id);
        $request_body = [
        ...
        if ($request->has('status') && strtolower($request->get('status')) == 'success') {
            ...
            if (isset($new_safelinks) && count($new_safelinks) > 0) {
                $safelinks = Safelink::whereIn('id', $new_safelinks)->get();
                foreach ($safelinks as $safelink) {
                    ...
                    $bonds_request = [
                        ...
                        'aggregator' => config('constant.SDWAN_Aggregator_URL'),
                        'SDWAN' => config('constant.SDWAN'),
                    ];
                    ...
                    if($bresult){
                        ...
                        $mailData = [
                            'customer_email' => $billing_profile->user->email,
                            'customer_name' => $billing_profile->user->name,
                            'product_name' => isset($safelink->master_product->Name) ? $safelink->master_product->Name : '',
                            'product_purchase_date' => $safelink->created_at,
                            'safelink_id' => $bresult['circuit_id'],
                            'safelink_name' => $bresult['name'],
                            'company_name' => $billing_profile->company_name,
                            'address' => $safelink->address,
                            'node_key' => $bresult['bonder']['key']
                        ];
                        Mail::to($mailData['customer_email'])->send(new \App\Mail\SafelinkDetailEmail($mailData));
                    }
    ...
    public function testBond($bond_id){
        ...
        curl_setopt($ch, CURLOPT_USERPWD, constant('constant.SDWAN.email').':'.constant('constant.SDWAN.password'));
