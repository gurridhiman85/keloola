...
class Helper
{
    ...
    public static function applyFilters($filters, $query, $type)
    {
        ...
        if ($txtSearch != "") {
            ...
            if($type == 'top_features'){
                $query->where('text', 'LIKE', "%{$txtSearch}%");
            }
            if($type == 'hardware_integrations'){
                $query->where('title', 'LIKE', "%{$txtSearch}%");
                $query->orWhere('description', 'LIKE', "%{$txtSearch}%");
                $query->orWhere('url', 'LIKE', "%{$txtSearch}%");
            }
            if($type == 'pricing'){
                $query->where('detailed_title', 'LIKE', "%{$txtSearch}%");
                $query->orWhere('description', 'LIKE', "%{$txtSearch}%");
            }
            if($type == 'knowledge_base'){
                $query->where('title', 'LIKE', "%{$txtSearch}%");
                $query->orWhere('question', 'LIKE', "%{$txtSearch}%");
                $query->orWhere('answer', 'LIKE', "%{$txtSearch}%");
                $query->orWhere('description', 'LIKE', "%{$txtSearch}%");
            }
            if($type == 'news_videos'){
                $query->where('title', 'LIKE', "%{$txtSearch}%");
                $query->orWhere('date', 'LIKE', "%{$txtSearch}%");
                $query->orWhere('description', 'LIKE', "%{$txtSearch}%");
            }
